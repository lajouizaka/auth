...
class EmailVerificationRequest
{
    public static function save(int $user_id)
    {
        ...
        $stm->execute([$user_id, $hash, time(), 0]);
        return [
            "id" => DB::lastID(),
            "verif_code" => $hash
    ];
    ...
    {
        $stm = DB::query("SELECT user_id,timestamp,verif_code from requests WHERE id=? AND type=0 LIMIT 1;");
        $stm->execute([$id]);
        return $stm->fetch();
    }
    ...
    {
        $stm = DB::query("DELETE FROM requests WHERE user_id = ? and  type = 0");
        $stm->execute([$id]);
        return $stm->rowCount();
    }
}
